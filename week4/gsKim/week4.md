...
```
###slice란?
프로세스들을 논리적으로 묶어 관리하는 최상위 그룹이다. 
이는 자원 격리와 공정성을 위해 사용된다. 
계층적 자원 제한: 여러 서비스나 프로세스를 하나의 슬라이스로 묶어, 해당 그룹 전체가 사용할 수 있는 CPU나 메모리의 최대치를 제한할 수 있다.
자원 격리: 특정 슬라이스(예: 사용자 작업)가 과도하게 자원을 사용하더라도 다른 슬라이스의 작동에 방해가 되지 않도록 격리한다.

systemd 기반 시스템은 모든 프로세스를 무질서하게 두지 않고, 크게 세 가지 Slice로 분리하여 관리한다. 
